    ...
    while (nextBlock >= config.startBlock && count > 0) {
      web3.eth.getBlock(nextBlock, true, function (error, blockData) {
        if (error) {
          ...
          for (let i = 0; i < blockData.uncles.length; i++) {
            web3.eth.getUncle(blockData.uncles[i], i, false, function (error, uncleData) {
              if (error) {
                console.log('Warning: error on getting uncle with hash/number: ' + blockData.uncles[i] + ': ' + error);
              } else if (uncleData == null) {
                console.log('Warning: null uncle data received from the uncle with hash/number: ' + blockData.uncles[i]);
              } else {
                uncleData.blockNumber = blockData.number;
                uncleData.blockHash = blockData.hash;
                uncleData.position = i;
                writeUncleToDB(config, uncleData, true);
              }
            })
          }
        }
...
var writeBlockToDB = function (config, blockData, flush) {
  ...
  if (blockData && blockData.number >= 0) {
    blockData.transactions = blockData.transactions.length;
...
/**
 Write the whole uncle object to DB
 **/
var writeUncleToDB = function (config, blockData, flush) {
  var self = writeUncleToDB;
  if (!self.bulkOps) {
    self.bulkOps = [];
  }
  if (blockData && blockData.number >= 0) {
    self.bulkOps.push(new Uncle(blockData));
    console.log('\t- uncle #' + blockData.number.toString() + ' inserted.');
  }
  ...
  if (flush && self.bulkOps.length > 0 || self.bulkOps.length >= config.bulkSize) {
    ...
    Uncle.collection.insert(bulk, function (err, blocks) {
      if (typeof err !== 'undefined' && err) {
        if (err.code == 11000) {
          if (!('quiet' in config && config.quiet === true)) {
            console.log('Skip: Duplicate DB key : ' + err);
        ...
        console.log('* ' + blocks.insertedCount + ' uncles successfully written.');
      }
    });
  }
}
